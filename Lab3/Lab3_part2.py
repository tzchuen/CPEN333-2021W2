...
# renames multiprocessing for brevity in code
...
if __name__ == "__main__":
    ...
    for i in range(SIZE):  #checking all columns

        # creats process objects for checking rows, columns, subgrids
        colMP = mp.Process(target = checkColumn, args=(testcase, i))
        rowMP = mp.Process(target = checkRow, args=(testcase, i))
        subgridMP = mp.Process(target = checkSubgrid, args=(testcase, i))
